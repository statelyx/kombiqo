...
function ProductHanger({ category }: { category: Garment['category'] }) {
  if (['Ayakkabılar', 'Çantalar', 'Aksesuarlar'].includes(category)) return null;
  return <><Path d="M75 14 C75 4 88 5 87 13 C87 18 80 18 80 25 L80 29" fill="none" stroke="#AB8B67" strokeWidth="2" />
    {category === 'Altlar' ? <><Path d="M80 27 L80 36 M44 36 L116 36" stroke="#AB8B67" strokeWidth="3" strokeLinecap="round" /><Path d="M47 35 L47 46 L55 46 L55 35 M105 35 L105 46 L113 46 L113 35" fill="#AB8B67" stroke="#8F7354" /></> : <Path d="M80 27 L44 46 Q42 49 47 49 L113 49 Q118 49 115 46 Z" fill="none" stroke="#AB8B67" strokeWidth={category === 'Elbiseler' ? 2 : 3} />}
  </>;
}
function GarmentArtBase({ item, size = 130, hanger = false }: { item: Garment; size?: number; hanger?: boolean }) {
  ...
  if (item.image) return <View accessible accessibilityLabel={item.name} style={{ width: size, height: size * 1.2, backgroundColor: item.backdrop === 'Taş' ? '#E0DFDB' : item.backdrop === 'Krem' ? '#F2EDE3' : 'transparent', borderRadius: 16 }}>
    {item.cutout && <Svg width={size} height={size * 1.2} viewBox="0 0 160 192" style={{ position: 'absolute' }}>
      ...
      {hanger && <ProductHanger category={item.category} />}
    </Svg>}
    <View style={{ marginTop: item.cutout && hanger ? size * .21 : 0, alignItems: 'center' }}><ProductImage image={item.category === 'Ayakkabılar' && item.preferredAngle === 'second' && item.secondImage ? item.secondImage : item.image} strokes={item.preferredAngle === 'second' ? [] : item.brush} width={item.cutout && hanger ? size * .8 : size} height={item.cutout && hanger ? size * .96 : size * 1.2} /></View>
  </View>;
  return <View accessible accessibilityLabel={item.name}><Svg width={size} height={size * 1.2} viewBox="0 0 160 192">
    ...
    {item.category === 'Altlar' && item.subtype !== 'Etek' && <><Path d="M48 47 L112 47 L121 174 L88 174 L79 97 L72 174 L39 174 Z" fill={`url(#${gradientId})`} stroke="#403A32" strokeOpacity="0.2" /><Path d="M48 55 L113 55 M80 56 L80 91 M53 61 Q54 79 44 85 M107 61 Q106 78 116 85 M61 69 L54 167 M98 68 L105 167" stroke="#403A32" strokeOpacity="0.23" strokeWidth="1.5" fill="none" /></>}
    {item.category === 'Altlar' && item.subtype === 'Etek' && <Path d="M51 47 L109 47 L134 173 Q80 181 26 173 Z" fill={item.color} stroke="#403A3230" />}
    {item.category === 'Çantalar' && <><Path d="M56 80 C56 30 104 30 104 80" fill="none" stroke={item.color} strokeWidth="8" /><Path d="M35 78 L125 78 L133 160 Q80 175 27 160 Z" fill={item.color} stroke="#403A3240" /></>}
    {item.category === 'Aksesuarlar' && <><Path d="M45 55 Q80 190 115 55" fill="none" stroke={item.color} strokeWidth="8" /><Ellipse cx="80" cy="128" rx="12" ry="16" fill={item.color} /></>}
