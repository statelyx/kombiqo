  ...
  return <View style={s.panel}>
    ...
    <Modal visible={phase !== 'idle'} animationType="fade" onRequestClose={close}>
      <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content}>
        <Text style={s.kicker}>AKILLI EKLE</Text><Text style={s.heading}>{phase === 'scanning' ? 'Parçan hazırlanıyor.' : 'Son dokunuş sende.'}</Text>
        <View style={s.preview}>{photo && <Image source={{ uri: photoUri(photo) }} resizeMode="contain" style={s.image} accessibilityLabel="İncelenen kıyafet" />}{phase === 'scanning' && <Animated.View pointerEvents="none" style={[s.scan, { transform: [{ translateY: motion.interpolate({ inputRange: [0, 1], outputRange: [-90, 280] }) }] }]} />}</View>
