...
import { Preferences, Profile, STYLES, Style } from './domain';
...
export function SelectList({ label, options, values, onChange, multiple = false, custom = false }: { label: string; options: readonly string[]; values: string[]; onChange: (values: string[]) => void; multiple?: boolean; custom?: boolean }) {
  ...
  function choose(value: string) { onChange(multiple ? values.includes(value) ? values.filter(x => x !== value) : [...values, value] : [value]); if (!multiple) setOpen(false); }
...
export function ProfileEditor({ profile, preferences, onSave }: { profile?: Profile; preferences: Preferences; onSave: (profile: Profile, styles: Style[]) => void }) {
  ...
  function submit() {
    if (age && (!/^\d{1,3}$/.test(age) || Number(age) < 1 || Number(age) > 120)) { setError('Yaşını 1–120 arasında yazabilir veya boş bırakabilirsin.'); return; }
  ...
  return <Modal visible animationType="slide" onRequestClose={submit}><SafeAreaView style={s.page}><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[s.content, { flex: undefined }]}><Text style={s.kicker}>SANA AİT BİR BAŞLANGIÇ</Text><Text style={s.title}>Seni biraz tanıyalım.</Text><Text style={s.text}>Seçtiklerin ilk önerileri şekillendirir. Kaydırdıkça birlikte keşfederiz.</Text>
    ...
    <Text style={[s.text, { marginVertical: 20, fontSize: 12 }]}>Bilgilerin bu cihazda kalır. Yaş ve cinsiyet tarzlarını sınırlandırmaz. Marka ve tarz seçimlerini sonra değiştirebilirsin.</Text>{!!error && <Text style={{ color: '#AF382E' }}>{error}</Text>}<Pressable accessibilityRole="button" style={s.button} onPress={submit}><Text style={s.white}>Tarzımı keşfet</Text></Pressable></ScrollView></KeyboardAvoidingView></SafeAreaView></Modal>;
}
const s = StyleSheet.create({ page: { flex: 1, backgroundColor: '#F8F6F1' }, content: { flex: 1, padding: 24, width: '100%', maxWidth: 760, alignSelf: 'center' }, title: { fontSize: 30, color: '#282F29', marginBottom: 16 }, kicker: { fontSize: 10, letterSpacing: 2, color: '#65715A', marginBottom: 18 }, label: { marginTop: 24, marginBottom: 10, fontSize: 13, color: '#65715A' }, text: { color: '#282F29', fontSize: 15, lineHeight: 23 }, input: { borderWidth: 1, borderColor: '#E6E5DD', borderRadius: 12, padding: 15, backgroundColor: 'white', fontSize: 16, color: '#282F29' }, option: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 17, borderBottomWidth: 1, borderColor: '#E6E5DD' }, button: { backgroundColor: '#282F29', borderRadius: 15, padding: 17, alignItems: 'center', marginVertical: 14 }, white: { color: 'white', fontWeight: '600' } });
